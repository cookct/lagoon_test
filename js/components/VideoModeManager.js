...
import { queueVideoApi, retrieveVideoApi } from '../api.js';
...
export class VideoModeManager {
    ...
    bindEvents() {
        ...
        // Model change -> refresh parameters
        this.dom.modelSelect.addEventListener('change', () => {
            if (window.state.mode === 'video') this.refreshParameterPanel();
        });
...
refreshParameterPanel() {
    // Never render video UI outside video mode
    if (window.state.mode !== 'video') return;
    ...
    async generateVideo() {
        ...
        if (payload.ref_images) {
            ...
            if (refs.length > 0) payload.reference_image_urls = refs;
        }
